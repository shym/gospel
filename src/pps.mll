{
 (** Kinds of space:
     - [Large] is one that would detach a documentation from a value in the
      OCaml compiler, i.e. one with at least one blank line (a newline followed
      by zero, one or more blanks and then a newline)
   ...
   | Ghost (start_p, end_p, g)
     ...
     when k <> Large ->
       Fmt.str "%s%s%s%s"
         ...
         (print_documentation_attribute `ThreeAt doc_start_p doc_end_p d)
         (print l)
   | Ghost (start_p, _, g) :: Spaces (k,_) :: Spec (inner_start_p, end_p, s) :: l
     when k <> Large ->
       Fmt.str "%s%s"
         (print_nested_gospel start_p inner_start_p end_p g s)
         (print l)
