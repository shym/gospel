{
  ...
  let print_directive where ppf pos =
    let open Lexing in
    match where with
    | `Open ->
  ...
  *)
  ...
  let print_nested_gospel start_p inner_start_p end_p outer_s inner_s ppf =
    Fmt.pf ppf "[@@@@@@gospel%a {|%s|}[@@@@gospel%a {|%s|}]%a]"
      (print_directive `Open) start_p outer_s (print_directive `Open)
      inner_start_p inner_s (print_directive `Close) end_p
  ...
  let print_empty_documentation end_p ppf =
    Fmt.pf ppf "[@@@ocaml.doc%a]" (print_directive `Close) end_p
...

{
  let run ppf lb =
    clear ();
    print_directive `Beginning ppf lb.Lexing.lex_curr_p;
    scan ppf lb
}

(* Note: to be able to reindent this file easily, the curly braces that surround
   OCaml code blocks are written on separate lines.

   Reindent this file in vim with:
    :g/ {$/+,/^    }$/-!ocamlformat --impl -m 74 - | sed 's/^./      &/'
   for OCaml code in rules;
    :g/^{/+,/^}$/-!ocamlformat --impl -m 78 - | sed 's/^./  &/'
   for OCaml code elsewhere.
*)
