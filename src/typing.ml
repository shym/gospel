...
let rec dterm kid crcm ns denv { term_desc; term_loc = loc } : dterm =
  ...
  let gen_app ~loc ls tl =
    (* gen_app in two layers, to check that constructors are fully
       applied without enforcing this on functions;
       note that this accepts both [C x y] and [C (x,y)] syntaxes for
       the application of the constructor [C] to its 2 arguments as it
       is the syntax used for tuples and (::) *)
